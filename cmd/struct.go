...
func (w *Weather) PrintWeather() {
	...
	time := time.Unix(w.Current.TimeOfUpdate, 0).Format("15:04")

	fmt.Printf("Location:[%s] , "+"Temp:[%0.fC] , "+"Humidity:[%d] , "+"FeelsLike:[%0.fC] , "+"UV:[%0.f] , "+"AQI:[%d] ,"+"Condition:[%s] ,"+"Rain:[%0.fmm] , "+"TimeOfUpdate: [%s] \n",
		w.Location.Name,
		w.Current.TemC,
		w.Current.Humidity,
		w.Current.FeelsLike,
